...
# 파일 추가 (파일 다이얼로그 / filedialog)
def add_file():
    current_path = os.path.dirname(__file__)
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
                                        filetypes=(("PNG 파일", "*.png"), ("모든 파일", "*.*")), \
                                        initialdir=current_path)  # 최초 사용자 지정 경로로 보여줌
...
def browse_dest_path():
    ...
    if folder_selected == '': # 사용자가 취소를 누를때
        return
    txt_dest_path.delete(0, END)  # text였으면 1.0 END
...
def merge_image():
    ...
    try:
        ...
        # 원복 width : 원본 heigth = 변경 width : 변경 height
        #    100    :    60       =     80    :     ?
        ...
        for idx, img in enumerate(images):
            ...
            y_offset += (img.size[1] + img_space)  # height 값 + 사용자 지정 간격 만큼 더해줌
...
txt_dest_path.pack(side="left", padx=5, pady=5, fill="x", expand=True, ipady=4)  # ipady:높이변경

btn_dest_path = Button(path_frame, padx=5, pady=5, text="찾아보기", width=10, command=browse_dest_path)
